...
class Profile extends Component {
  ...
  updateProfile = () => {
    this.setState({isLoading: true});
    this.setState({saveProfileStatus: <div><CircularProgress/></div>});
    ...
    api.put('profile/' + this.state.profileId,
      {
  ...
  deleteProfile = () => {
    ...
    this.setState({deleteProfileStatus: <div><CircularProgress/></div>});
    api.delete('/profile/' + this.props.match.params.id
    ).then((response) => {
    ...
    .catch(() => {
      this.setState({deleteProfileStatus: <div>An error has occurred!</div>});
  ...
  handleActiveCheckbox = (index, value, ruleType) => {
    let newState = [];
    if (ruleType === 'inbound') {
      newState = update(this.state.inboundRules, {[index]: {'active': {$set: value}}});
      this.setState({inboundRules: newState});
    } else {
      newState = update(this.state.outboundRules, {[index]: {'active': {$set: value}}});
  ...
  handleIntfSelect = (index, value, ruleType) => {
    let newState = [];
    if (ruleType === 'inbound') {
      newState = update(this.state.inboundRules, {[index]: {'interface': {$set: value}}});
      this.setState({inboundRules: newState});
    } else {
      newState = update(this.state.outboundRules, {[index]: {'interface': {$set: value}}});
      this.setState({outboundRules: newState});
    }
  ...
  handleGroupSelect = (index, value, ruleType) => {
    let newState = [];
    if (ruleType === 'inbound') {
      newState = update(this.state.inboundRules, {[index]: {'group': {$set: value}}});
      this.setState({inboundRules: newState});
    } else {
      newState = update(this.state.outboundRules, {[index]: {'group': {$set: value}}});
      this.setState({outboundRules: newState});
    }
  ...
  handleServiceSelect = (index, value, ruleType) => {
    let newState = [];
    if (ruleType === 'inbound') {
      newState = update(this.state.inboundRules, {[index]: {'service': {$set: value}}});
      this.setState({inboundRules: newState});
    } else {
      newState = update(this.state.outboundRules, {[index]: {'service': {$set: value}}});
      this.setState({outboundRules: newState});
    }
  ...
  handleActionSelect = (index, value, ruleType) => {
    let newState = [];
    if (ruleType === 'inbound') {
      newState = update(this.state.inboundRules, {[index]: {'action': {$set: value}}});
      this.setState({inboundRules: newState});
    } else {
      newState = update(this.state.outboundRules, {[index]: {'action': {$set: value}}});
      this.setState({outboundRules: newState});
    }
  ...
  handleLogCheckbox = (index, value, ruleType) => {
    let newState = [];
    if (ruleType === 'inbound') {
      newState = update(this.state.inboundRules, {[index]: {'log': {$set: value}}});
      this.setState({inboundRules: newState});
    } else {
      newState = update(this.state.outboundRules, {[index]: {'log': {$set: value}}});
      this.setState({outboundRules: newState});
    }
  ...
  handleLogPrefixInput = (index, value, ruleType) => {
    let newState = [];
    if (ruleType === 'inbound') {
      newState = update(this.state.inboundRules, {[index]: {'log_prefix': {$set: value}}});
      this.setState({inboundRules: newState});
    } else {
      newState = update(this.state.outboundRules, {[index]: {'log_prefix': {$set: value}}});
      this.setState({outboundRules: newState});
    }
  ...
  handleCommentInput = (index, value, ruleType) => {
    ...
    let newState = [];
    if (ruleType === 'inbound') {
      newState = update(this.state.inboundRules, {[index]: {'comment': {$set: value}}});
      this.setState({inboundRules: newState});
    } else {
      newState = update(this.state.outboundRules, {[index]: {'comment': {$set: value}}});
      this.setState({outboundRules: newState});
    }
  ...
  handleAddProfile = (ruleType) => {
    let newState = [];
    if (ruleType === 'inbound') {
      newState = update(this.state.inboundRules, {$push: [{'order': 1, 'active': false, 'interface': '', 'group': ' ', 'group_type': 'ANY', 'service': ' ', 'action': 'ACCEPT', 'log': false, 'log_prefix': '', 'comment': '', 'type': 'BASIC'}]});
      this.setState({inboundRules: newState});
    } else {
      newState = update(this.state.outboundRules, {$push: [{'order': 1, 'active': false, 'interface': '', 'group': ' ', 'group_type': 'ANY', 'service': ' ', 'action': 'ACCEPT', 'log': false, 'log_prefix': '', 'comment': '', 'type': 'BASIC'}]});
      this.setState({outboundRules: newState});
    }
  }
  ...
  handleRemoveProfile = (index, ruleType) => {
    ...
    if (ruleType === 'inbound') {
      ...
      if (newState.length === 0) {
        newState.push({'order': 1, 'active': false, 'interface': '', 'group': ' ', 'group_type': 'ANY', 'service': ' ', 'action': 'ACCEPT', 'log': false, 'log_prefix': '', 'comment': '', 'type': 'BASIC'});
      }
      this.setState({inboundRules: newState});
    } else {
  ...
  render(props) {
    ...
    } else if (this.state.hasErrored || this.props.servicesHasErrored || this.props.groupsHasErrored) {
        return <p>Sorry! There was an error loading the items</p>;
    }
    if (this.state.isLoading || this.props.servicesIsLoading || this.props.groupsIsLoading) {
    ...
    return (
      ...
      <Paper className={classes.root}>
        <Table className={classes.table}>
          ...
          <TableBodySortable children={this.state.inboundRules} ruleType={"inbound"} onSortEnd={this.onSortEndInbound} groups={this.props.groups} services={this.props.services} useDragHandle handleActiveCheckbox={this.handleActiveCheckbox} handleIntfSelect={this.handleIntfSelect}
      ...
      <Paper className={classes.root}>
        <Table className={classes.table}>
          ...
          <TableBodySortable children={this.state.outboundRules} ruleType={"outbound"} onSortEnd={this.onSortEndOutbound} groups={this.props.groups} services={this.props.services} useDragHandle handleActiveCheckbox={this.handleActiveCheckbox} handleIntfSelect={this.handleIntfSelect}
            handleGroupSelect={this.handleGroupSelect}
            handleServiceSelect={this.handleServiceSelect}
            handleActionSelect={this.handleActionSelect}
            handleLogCheckbox={this.handleLogCheckbox}
      ...
      >
        <DialogTitle id="form-dialog-title">Confirm save of profile: {this.state.profileName}?</DialogTitle>
        <DialogContent>
          <DialogContentText>
            Are you sure you want to save the changes to profile: {this.state.profileName}?
          </DialogContentText>
        </DialogContent>
        <DialogActions>
          ...
          <Button onClick={this.updateProfile} variant="contained" color="primary">
            Submit Change
          </Button>
      ...
      <Dialog
        ...
        aria-labelledby="form-dialog-title"
      >
        <DialogTitle id="form-dialog-title">Confirm delete of profile: {this.state.profileName}?</DialogTitle>
        <DialogContent>
          <DialogContentText>
        ...
        <DialogActions>
          ...
          <Button onClick={this.deleteProfile} variant="contained" color="primary">
            Delete
            <DeleteIcon className={classes.rightIcon}/>
          </Button>
