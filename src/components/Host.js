...
class Host extends Component {
    ...
    updateHost = () => {
      this.setState({isLoading: true});
      this.setState({editHostStatus: <div><CircularProgress className={this.props.classes.progress}/></div>});
      api.put('/host/' + this.state.hostId, {
    ...
    deleteHost = () => {
      ...
      this.setState({deleteHostStatus: <div><CircularProgress className={this.props.classes.progress}/></div>});
      api.delete('/host/' + this.props.match.params.id
      ).then((response) => {
        if (response.status === 204) {
          this.setState({isDeleting: false});
    ...
    render() {
      ...
      if (this.state.isLoading || this.props.groupsIsLoading || this.state.isDeleting) {
      ...
      return (
        <div>
          ...
          >
            <DialogTitle id="form-dialog-title">Confirm edit of host: {this.state.hostName}?</DialogTitle>
            <DialogContent>
              <DialogContentText>
                Are you sure you want to change the group to: {this.state.hostGroupName}?
              </DialogContentText>
            </DialogContent>
            <DialogActions>
              ...
              <Button onClick={this.updateHost} variant="contained" color="primary">
                Submit Change
              </Button>
          ...
          <Dialog
            ...
            aria-labelledby="form-dialog-title"
          >
            <DialogTitle id="form-dialog-title">Confirm delete of: {this.state.hostName}?</DialogTitle>
            <DialogContent>
              <DialogContentText>
            ...
            <DialogActions>
              ...
              <Button onClick={this.deleteHost} variant="contained" color="primary">
                Delete
                <DeleteIcon className={classes.rightIcon}/>
              </Button>
              &nbsp;&nbsp;{this.state.deleteHostStatus}
...
export default connect(mapStateToProps, mapDispatchToProps)(withRouter(withStyles(styles)(Host)));
